...
The overall approximate API coverage (with the caveats above) is 49.8%. We prioritize implementing commonly used functions, as pyspark has many esoteric APIs.
...
| [`pyspark.sql.functions.regexp_extract_all`](/pyspark-dubber/API Reference/pyspark.sql.functions/pyspark.sql.functions.regexp_extract_all) | :material-check: | For technical reasons, the first argument is called `str_` instead of `str`. |
...
| [`pyspark.sql.functions.struct`](https://spark.apache.org/docs/latest/api/python/reference/pyspark.sql/api/pyspark.sql.functions.struct.html) | :material-check: |  |
